...
* BeforeSave
* AfterSave
* AfterInsert (new item is being created, available since v8.3)
...
* **WaitOutsideTransactionAndFailOnError**  The executable process will be launched inside the transaction repeater and the process handle will be saved. Later, after the transaction finishes its work, the saving routine will wait for the process handle to end. If an error occurs, the saving / removing transaction is already committed, but error ReturnCode will be returned. The saving / removing routine does not end before the trigger process finishes (eWay-CRM synchronization that caused the operation is waiting for the end).
...
> [!NOTE]
For Jobs (ScheduledAtTime triggers) use always this `WaitingType="WaitInTransaction"`.
